...
def collect_metrics():
    # In real implementation, query the CogniMesh controller API
    for sm_id in range(25600):
        warp_occupancy.labels(sm_id=sm_id).set(random.random())
        tensor_tflops.labels(sm_id=sm_id).set(random.uniform(0.1, 0.5))
